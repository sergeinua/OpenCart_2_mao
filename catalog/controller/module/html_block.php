	...
	public function index($block) {
		...
		if (isset($block['style']) && !empty($block['css'])) {
			...
			if (file_exists(DIR_APPLICATION .  'view/theme/' . $this->config->get('config_template') . '/stylesheet/html_block.css')) {
				$file_name = $this->config->get('config_template') . '/stylesheet/html_block.css';
			} else {
				$file_name = 'default/stylesheet/html_block.css';
			}
			
			$this->document->addStyle('catalog/view/theme/' . $file_name);
		}
